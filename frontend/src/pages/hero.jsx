...
export default function Example() {
  ...
  return (
    <div className="bg-white" id="home-section">
      <header className="absolute inset-x-0 top-0 z-50">
        <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
          ...
          <div className="hidden lg:flex lg:gap-x-12">
            {navigation.map((item) => (
              <a key={item.name} href={item.href} className="text-xl/15 font-semibold text-gray-900">
                {item.name}
              </a>
            ))}
          </div>
                ...
                <div className="space-y-2 py-6">
                  {navigation.map((item) => (
                    <a
                      key={item.name}
                      href={item.href}
                      onclick={(e)=>{e.preventDefault();scrollToSection(item.href);setMobileMenuOption(false);}}
      ...
      <div className="relative isolate px-6 pt-14 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl py-32 sm:py-48 lg:py-48">
          ...
          <div className="text-center">
            <h1 className="md:text-5xl font-semibold tracking-tight text-balance text-gray-900 sm:text-7xl">
              Your Daily Dose of Digital Sunshine. Let's Chat!
            </h1>
            <p className="mt-8 text-lg font-medium text-pretty text-gray-500 sm:text-xl/8">
              Stuck in a rut? Open our free, friendly AI chat for a joke, fun fact, or just a good talk. Your pocket friend awaits!
            </p>
